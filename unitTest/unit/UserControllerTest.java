...
public class UserControllerTest {
    ...
    public void updateUserByID_requestUserDTOWithWrongEmailToBeUpdated_returnBadRequest() throws Exception {
        String emailBeforeTest = userDTOToBeTestedAgainst.getEmail();
        userDTOToBeTestedAgainst.setEmail("this is a wrong email");
        String userDTOAsJsonString = new ObjectMapper().writeValueAsString(userDTOToBeTestedAgainst);
        ...
        ArgumentCaptor<User> stringArgumentCaptor = ArgumentCaptor.forClass(User.class);
        ArgumentCaptor<User> userArgumentCaptor = ArgumentCaptor.forClass(User.class);

        verify(userService, times(0)).save(stringArgumentCaptor.capture());
        userDTOToBeTestedAgainst.setEmail(emailBeforeTest);
    }
